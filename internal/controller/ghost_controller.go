...
type GhostReconciler struct {
	...
	// below kubebuilder markers tells the manager which and what kind of access it has to the mentioned resource
}
...
func (r *GhostReconciler) addPvcIfNotExists(ctx context.Context, ghost *blogv1.Ghost) error {
	...
	if err == nil {
		// PVC exists, we are done here and not creating it
		return nil
	}
	...
	// setting the ghost resource as a reference for the pvc which means deleting the ghost resource will also delete the pvc referencing it
...
func (r *GhostReconciler) addOrUpdateDeployment(ctx context.Context, ghost *blogv1.Ghost) error {
	...
	desiredDeployment := generateDesiredDeployment(ghost)
	// setting the ghost resource as a reference for the deployment which means deleting the ghost resource will also delete the deployment referencing it
...
func (r *GhostReconciler) addServiceIfNotExists(ctx context.Context, ghost *blogv1.Ghost) error {
	...
	// setting the ghost resource as a reference for the service which means deleting the ghost resource will also delete the service referencing it
...
func (r *GhostReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Applying the ghost resource(custom resource) creates the other resources like pvc, deployment, service needed for the ghost resource to operate which was defined as kubebuilder marker for the manager.
	pvcReady := false
